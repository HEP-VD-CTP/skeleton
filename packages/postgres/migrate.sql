...
-- This file is executed by the backend on every startup.
-- It must use only idempotent statements (IF NOT EXISTS, OR REPLACE, ON CONFLICT DO NOTHING).
-- When adding a new table or index to db.sql, add the corresponding idempotent statement here.
